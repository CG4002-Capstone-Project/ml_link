...
class DataCollector:
    ...
    def insert_acc_data(self, timestamp, dancer, accx, accy, accz):
        rd = {
            "measurement": "acc",
            "tags": {"dancer": str(dancer)},
            "timestamp": str(timestamp),
            "fields": {"x": accx, "y": accy, "z": accz},
        }
    ...
    def insert_gyr_data(self, timestamp, dancer, yaw, pitch, roll):
        rd = {
            ...
            "tags": {"dancer": str(dancer)},
            "timestamp": str(timestamp),
            "fields": {"yaw": yaw, "pitch": pitch, "roll": roll},
        }
        self.insert_raw_data(rd)
...
if __name__ == "__main__":
    ...
    for i in range(1000):
        ...
        data_collector.insert_acc_data(
            int(time.time()), 1, math.sin(i), math.sin(i + 3), math.sin(i + 6)
        )
        data_collector.insert_gyr_data(
            int(time.time()), 1, math.sin(i), math.sin(i + 3), math.sin(i + 6)
        )
        time.sleep(0.1)
